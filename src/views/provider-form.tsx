...
import lmstudioConfig from "../providers/lmstudio/config";
...
const providers: { value: string; title: string; config: IConfig }[] = [
  ...
  { value: "lmstudio", title: "LM Studio", config: lmstudioConfig },
];
...
export const ProviderForm = (props: ProviderFormProps) => {
  ...
  const fetchModels = async () => {
    try {
      ...
      const notFound = !models.find((m) => m.id === _model);
      if (notFound && models.length > 0) {
  ...
  function handleProviderChange(provider: string) {
    ...
    if (selectedProvider) {
      ...
      setEntrypoint(selectedProvider.config.defaultEntrypoint);
